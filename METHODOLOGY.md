...
**Headline counts count evidenced rows only.** On the receipts as at 2026-09-10: **96 evidenced · 35 candidate** across the 131 rows — 47 rows by first-party incident, 20 more by corpus coding alone, and 29 by a public field case, the third receipt type, first exercised on 2026-09-09 when research 98 filed dated, source-checked citations for 26 rows and `check-registers.py` began deriving that receipt from the file rather than from a hand-typed constant, then FF-2026.3 drained three more field-case-evidenced rows on 2026-09-10. (Before that the pair read 67 · 61 and had not moved since 21 August; on 22 August `CL-5B` gained a first-party mapping it did not have, which moves a row between the two evidence *sources* without moving the tier split, since it was already evidenced by corpus coding.) The split per register is in the [registers README](README.md) table.
