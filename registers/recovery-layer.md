...
## Stage 1 · CAPTURED — is there a point to go back to? (0 prevented · 2 recoverable · 4 irreversible)

| ID | Evidence | Sev | Failure | Shows up as | Prevention | Catch | Mechanism | Tool | Outcome |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| RL-1A | evidenced | S3 | No restore point between the last good state and now | “It worked an hour and forty edits ago” | Automatic checkpoint before each user prompt | prompt | Claude Code checkpoints file-tool edits; can itself track zero files | Claude Code · automatic · available | C irreversible |
| RL-1B | evidenced | S4 | Work happened where Git cannot see it | “It edited a file the repo has never held” | Keep every writable path under version control | on disk | Untracked-path audit on the working set | Git · maintained · available | B recoverable |
| RL-1C | evidenced | S4 | The data changed with no restore point | “The migration ran; the old rows are gone” | Snapshot before any schema or data change | pre-tool | Snapshot or branch gate before migrate | Supabase · maintained · available | B recoverable |
...
**Regraded from Class A to Class C on 2026-09-10, on the field record.** `FC-20` (Claude Code issue #14002) shows the checkpoint holding zero tracked files while file modifications were tracked against it, so `/rewind` had nothing to restore. A field case in which the named control failed is never Class A. The stricter reading is taken here rather than Class B: the row's own mechanism is the checkpoint, and the checkpoint is what the reproduction shows failing outright, not partially — nothing the row itself credits gets the work back. Git, named in the paragraph above as a path outside this row's boundary, is a separate practice this row does not count as its mechanism. The cross-register split of 11 prevented · 73 detected/recoverable · 44 survives/irreversible moves to 10 · 73 · 45 with this regrade alone, before IL-6D, AL-4F and CL-3D are added.
...
| ID | Evidence | Sev | Failure | Shows up as | Prevention | Catch | Mechanism | Tool | Outcome |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| RL-2A | evidenced | S4 | The agent could write anywhere on the machine | “It tidied a folder you never named” | Bound the writable surface before the turn starts | on disk | Agent-editable settings do not bound the OS principal | Claude Code · automatic · available | C irreversible |
| RL-2B | evidenced | S4 | Permissions were waived for the whole session | “It was faster with the prompts off” | Waive per action, never per session | prompt | Skip-permissions only inside a sandbox | none · process · none | C irreversible |
...
| RL-4B | evidenced | S4 | History was rewritten over the only copy | “Force-pushed, and the old commits are gone” | Refuse force-push on shared branches | on disk | Force-push blocked server-side | Branch rules · maintained · available | A prevented |
...
| RL-5B | evidenced | S4 | The restore was never once tested | “The backups existed; none of them worked” | Restore on a schedule, not on an incident | review | Scheduled restore drill that must pass, dated, with a time on it | Cron · maintained · available | B recoverable |
...
| ID | Evidence | Sev | Failure | Shows up as | Prevention | Catch | Mechanism | Tool | Outcome |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| RL-6B | evidenced | S4 | The restore point is older than you think | “The last good snapshot is from March” | Alert on snapshot age, not on its existence | CI | Snapshot freshness check | Supabase · maintained · available | B recoverable |
...
## The eleven that still reach you

- RL-1A — a checkpoint verified to have tracked the files it claims to protect
...
Four are decisions nobody has written down. Three are orchestration gaps — the work exists and nothing carries it across the handoff. Four are boundaries the agent can still reach around, or a control that can silently fail: an unverified checkpoint, one writable surface per agent, a host sandbox it cannot edit, and a refusal that covers only the destructive shapes somebody enumerated.
...
*RL-1A added 2026-09-10: regraded from `A prevented` to `C irreversible` on the field record. FC-20 (Claude Code issue #14002) shows the checkpoint holding zero tracked files, so `/rewind` had nothing to restore — a field case in which the named control failed is never Class A, and nothing the row itself credits gets the work back.*
